...
COLORS = [RED, GREEN, IVORY, YELLOW, BLUE] = range(5)
NATIONALITIES = [ENGLISHMAN, SPANIARD,
                 UKRAINIAN, NORWEGIAN, JAPANESE] = range(5)
DRINKS = [COFFEE, TEA, MILK, ORANGE_JUICE, WATER] = range(5)
SMOKES = [OLD_GOLD, KOOLS, CHESTERFIELDS, LUCKY_STRIKE, PARLIAMENTS] = range(5)
PETS = [DOG, SNAILS, FOX, HORSE, ZEBRA] = range(5)

NATIONALITY_NAMES = ['Englishman', 'Spaniard',
                     'Ukrainian', 'Norwegian', 'Japanese']


def solution():
    _, n, d, _, p = next(filter(check, itertools.product(filter(lambda c: c[1] == COLORS[BLUE],
    ...
    return 'It is the {} who drinks the water.\nThe {} keeps the zebra.'.format(NATIONALITY_NAMES[n[d.index(DRINKS[WATER])]], NATIONALITY_NAMES[n[p.index(PETS[ZEBRA])]])


def check(sol):
    c, n, d, s, p = sol
    return (n.index(NATIONALITIES[ENGLISHMAN]) == c.index(COLORS[RED])
