...
def evaluate(input_data):
    ...
    for command in input_data:
        ...
        if command.startswith(': '):
            ...
            definition = ' '.join(
                name_to_definition.get(token, token)
                for token in tokens[1:]
            )
            name_to_definition[name] = definition
        else:
            for token in tokenize(replace(command, name_to_definition)):
                if is_number(token):
                    number = int(token)
                    stack.append(number)
                elif token == '+':
...
def replace(command, name_to_definition):
    return ' '.join(map(lambda token: name_to_definition.get(token, token), tokenize(command)))
