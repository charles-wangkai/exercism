...
class Yacht {
	...
	int score() {
		...
		} else if (yachtCategory == YachtCategory.FULL_HOUSE && isFullHouse()) {
			return Arrays.stream(dice).sum();
		} else if (yachtCategory == YachtCategory.FOUR_OF_A_KIND && isFourOfAKind()) {
			return dice[1] * 4;
		} else if (yachtCategory == YachtCategory.LITTLE_STRAIGHT && isLittleStraight()) {
			return 30;
		} else if (yachtCategory == YachtCategory.BIG_STRAIGHT && isBigStraight()) {
			return 30;
		} else if (yachtCategory == YachtCategory.CHOICE) {
			return Arrays.stream(dice).sum();
		} else if (yachtCategory == YachtCategory.YACHT && isYacht()) {
			return 50;
		} else {
			return 0;
		}
	...
	boolean isFullHouse() {
		return dice[0] == dice[1] && dice[3] == dice[4] && dice[0] != dice[4]
