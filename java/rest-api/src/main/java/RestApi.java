...
public class RestApi {
    ...
    public String get(String url) {
        if (url.equals("/users")) {
            return getUsers(userToLedger.keySet().stream().sorted().collect(Collectors.toList()));
        }
    ...
    public String get(String url, JSONObject payload) {
        if (url.equals("/users")) {
            return getUsers(
                    payload.getJSONArray("users").toList().stream().map(String::valueOf).collect(Collectors.toList()));
        }
    ...
    public String post(String url, JSONObject payload) {
        ...
        if (url.equals("/iou")) {
            ...
            return getUsers(Stream.of(lender, borrower).sorted().collect(Collectors.toList()));
        }
    ...
    private String getUsers(List<String> users) {
        return new JSONObject()
                .put("users", new JSONArray(users.stream().map(this::buildUserOutput).collect(Collectors.toList())))
                .toString();
    }
    ...
    private JSONObject buildUserOutput(String user) {
        ...
        return new JSONObject().put("name", user)
                .put("owes",
                        new JSONObject(ledger.keySet().stream().filter(other -> ledger.get(other) < 0)
                                .collect(Collectors.toMap(Function.identity(), other -> -ledger.get(other)))))
                .put("owedBy",
                        new JSONObject(ledger.keySet().stream().filter(other -> ledger.get(other) > 0)
                                .collect(Collectors.toMap(Function.identity(), other -> ledger.get(other)))))
                .put("balance", ledger.values().stream().mapToDouble(x -> x).sum());
    }
}