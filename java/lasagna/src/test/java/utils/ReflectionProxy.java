...
public abstract class ReflectionProxy {
  ...
  /**
   * Checks if an existing Collection type has the parameterized type (Generics) as expected (eg.
   * List<String>)
   *
  ...
  public boolean isMethodReturnType(
      Class<?> returnType, Class<?> parameterizedType, String name, Class<?>... parameterTypes) {
    Class<?> targetClass = getTargetClass();
    if (targetClass == null || name == null) {
      return false;
    }
