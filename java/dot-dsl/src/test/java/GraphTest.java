...
public class GraphTest {
  ...
  public void testGraphWithOneNodeWithKeywords() {
    ...
    assertThat(graph.getNodes()).containsExactly(new Node("a", Map.of("color", "green")));
    assertThat(graph.getEdges()).isEmpty();
    assertThat(graph.getAttributes()).isEmpty();
  }
  ...
  public void testGraphWithOneEdge() {
    ...
    assertThat(graph.getNodes()).isEmpty();
    assertThat(graph.getEdges()).containsExactly(new Edge("a", "b"));
    assertThat(graph.getAttributes()).isEmpty();
  }
  ...
  public void testGraphWithOneEdgeWithKeywords() {
    ...
    assertThat(graph.getNodes()).isEmpty();
    assertThat(graph.getEdges()).containsExactly(new Edge("a", "b", Map.of("color", "blue")));
    assertThat(graph.getAttributes()).isEmpty();
  }
  ...
  public void testGraphWithAttributes() {
    Graph graph =
        new Graph(Map.of("foo", "1", "title", "Testing Attrs", "bar", "true"))
    ...
    assertThat(graph.getEdges())
        .containsExactlyInAnyOrder(new Edge("a", "b", Map.of("color", "blue")), new Edge("b", "c"));
