...
public class DominoesTest {
	...
	public void separateLoopsTest() throws ChainNotFoundException {
		...
		Domino[] dominoesArray = { new Domino(1, 2), new Domino(2, 3), new Domino(3, 1), new Domino(1, 1),
				new Domino(2, 2), new Domino(3, 3) };
		List<Domino> dominoesList = Arrays.asList(dominoesArray);
	...
	public void nineElementsTest() throws ChainNotFoundException {
		Dominoes dominoes = new Dominoes();
		Domino[] dominoesArray = { new Domino(1, 2), new Domino(5, 3), new Domino(3, 1), new Domino(1, 2),
				new Domino(2, 4), new Domino(1, 6), new Domino(2, 3), new Domino(3, 4), new Domino(5, 6) };
		List<Domino> dominoesList = Arrays.asList(dominoesArray);
	...
	private void assertEndDominoesMatch(List<Domino> outputDominoes) {
		...
		String errorMessage = "The left value of the first domino (" + leftValueOfFirstDomino
				+ ") needs to match the right value of the last domino (" + rightValueOfLastDomino + ").";
	...
	private void assertConsecutiveDominoes(List<Domino> dominoes) {
		for (int i = 0; i < dominoes.size() - 1; i++) {
			...
			String errorMessage = "The right value of domino number " + i + " (" + rightValueOfIthDomino
					+ ") needs to match the left value of domino number " + (i + 1) + " (" + leftValueOfNextDomino
					+ ").";
