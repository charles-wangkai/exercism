...
public class DominoesTest {
  ...
  public void disconnectedDoubleLoopTest() {
    ...
    Domino[] dominoesArray = {
      new Domino(1, 2), new Domino(2, 1), new Domino(3, 4), new Domino(4, 3)
    };
    List<Domino> dominoesList = Arrays.asList(dominoesArray);
  ...
  public void disconnectedSingleIsolatedTest() {
    ...
    Domino[] dominoesArray = {
      new Domino(1, 2), new Domino(2, 3), new Domino(3, 1), new Domino(4, 4)
    };
    List<Domino> dominoesList = Arrays.asList(dominoesArray);
  ...
  public void needBacktrackTest() throws ChainNotFoundException {
    ...
    Domino[] dominoesArray = {
      new Domino(1, 2), new Domino(2, 3), new Domino(3, 1), new Domino(2, 4), new Domino(4, 2)
    };
    List<Domino> dominoesList = Arrays.asList(dominoesArray);
  ...
  private void assertEndDominoesMatch(List<Domino> outputDominoes) {
    ...
    String errorMessage =
        "The left value of the first domino ("
  ...
  private void assertSameDominoes(List<Domino> inputDominoes, List<Domino> outputDominoes) {
    String errorMessage =
        "The number of dominoes in the input list ("
            + inputDominoes.size()
            + ") needs to match the number of dominoes in the output chain ("
            + outputDominoes.size()
            + ")";
  ...
  private void assertConsecutiveDominoes(List<Domino> dominoes) {
    for (int i = 0; i < dominoes.size() - 1; i++) {
      ...
      String errorMessage =
          "The right value of domino number "
              + i
              + " ("
              + rightValueOfIthDomino
              + ") needs to match the left value of domino number "
              + (i + 1)
              + " ("
