...
public class ZebraPuzzle {
  ...
  static boolean check(
      List<Color> colorPermutation,
      List<Nationality> nationalityPermutation,
      List<Drink> drinkPermutation,
      List<Smoke> smokePermutation,
      List<Pet> petPermutation) {
    return nationalityPermutation.indexOf(Nationality.ENGLISHMAN)
            == colorPermutation.indexOf(Color.RED)
        ...
        && nationalityPermutation.indexOf(Nationality.UKRAINIAN)
            == drinkPermutation.indexOf(Drink.TEA)
        ...
        && Math.abs(smokePermutation.indexOf(Smoke.CHESTERFIELDS) - petPermutation.indexOf(Pet.FOX))
            == 1
        && Math.abs(smokePermutation.indexOf(Smoke.KOOLS) - petPermutation.indexOf(Pet.HORSE)) == 1
        && smokePermutation.indexOf(Smoke.LUCKY_STRIKE)
            == drinkPermutation.indexOf(Drink.ORANGE_JUICE)
        && nationalityPermutation.indexOf(Nationality.JAPANESE)
            == smokePermutation.indexOf(Smoke.PARLIAMENTS);
  }
  ...
  static <T> void search(List<List<T>> permutations, T[] values, int index) {
    if (index == values.length) {
      permutations.add(List.copyOf(Arrays.asList(values)));
